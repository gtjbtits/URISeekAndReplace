...
class URISeekAndReplace {
    protected $protocols = "mailto|http";
...
//        \n 11.txt";
    ...
    public function __construct(array $params = []) {
        if (isset($params["regexes"]) && is_array($params["regexes"])) {
            $this->uriRegexes = $params["regexes"];
        } else {
            $this->uriRegexes["abstract"] = "/(?:(\w{2,16}):(?:\/){0,2})?(?:[a-z\x{00a1}-\x{ffff}0-9]{2,}[\.:@])+[^\s<>]{2,}/{$this->regexFlags}";
        } 
        if (isset($params["regex_parts"]) && is_array($params["regex_parts"])) {
            $this->uriRegexParts = $params["regex_parts"];
        } else {
...
//            var_dump("Handle {$link}. Protocol: {$protocol}");
    ...
    protected function replaceHttps($src) {
        return $this->replaceHttp($src);
    }
    protected function replaceFtp($src) {
        return $this->replaceHttp($src);
    }
    protected function replaceSftp($src) {
        return $this->replaceHttp($src);
    }
    protected function replaceFtps($src) {
        return $this->replaceHttp($src);
    }
...
//            var_dump("{$src} does not fit MAIL");
    ...
    protected function replace($needle, $protocol, $haystack) {
        $replaced = $this->generateHtmlLink(($protocol . $needle), $needle);
     ...
     * @return string
     */
    protected function replaceUnknown($src) {
...
//            var_dump("Try handle {$src} via protocol: {$p}");
    ...
    protected function callReplacer($protocol, $src) {
    ...
    protected function generateHtmlLink($href, $text) {
